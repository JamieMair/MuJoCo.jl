...
function Base.getproperty(x::Model, f::Symbol)
    internal_pointer = x.internal_pointer
...
function Base.getproperty(x::Data, f::Symbol)
    ...
    f === :warning && return unsafe_load(Ptr{NTuple{8, mjWarningStat_}}(internal_pointer + 48))
    f === :timer && return unsafe_load(Ptr{NTuple{13, mjTimerStat_}}(internal_pointer + 112))
    f === :solver && return unsafe_load(Ptr{NTuple{1000, mjSolverStat_}}(internal_pointer + 320))
