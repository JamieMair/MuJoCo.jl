...
function extract_blocks(io, fileloc)
    fw = open(fileloc, "w")
    println(fw, "module WrappedFunctions")

    # TODO: Add line that says the file is autogenerated
    ...
    println(fw, "const _wrapped_fns = (" * join(fn_names, ", ") * ")")
    ...
    println(fw, "end # module WrappedFunctions")
    close(fw)
    nothing
end
